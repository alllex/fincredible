    ...
    $.widget('rs.carousel', {
        ...
        _addPagination: function () {
            ...
            for (i = 0; i < noOfPages; i++) {
                var title = $(self.elements.items[i]).attr('title');

                if (title){
                    links[i] = '<li class="' + fullName + '-pagination-link"><a href="#page-' + i + '">' + title + '</a></li>';
                }else {
        ...
        _setPaginationWidth: function(){
            var elems = this.elements,
                width = elems.pagination.parent().outerWidth();

            if(elems.prevAction.outerWidth()) {
                width -= elems.nextAction.outerWidth();
            }

            if(elems.nextAction.outerWidth()) {
                width -= elems.nextAction.outerWidth();
            }

            elems.pagination.width(width);
        },
